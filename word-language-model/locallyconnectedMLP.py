...
class LocallyConnectedMLP(nn.Module):
    def __init__(self,
        ...
        # seq size x batch size x embeddings -> reduce size of individual embeddings
        i = 0
        if self.conv:
            conv_name = 'conv1'
            if kernel_size[0] % 2 == 0:
                kernel_size[0] -= 1
            p = kernel_size[0] // 2
            lcstack.append((conv_name, nn.Conv1d(
                1, output_dim[0], kernel_size[0], stride[0], p)))
            i = 1
        ...
        for j in range(i, n_layers-1):
            ...
            for arg in [input_dim, output_dim, kernel_size, stride]:
                args.append(arg[j])
            lc = LocallyConnectedLayer1d(
                args[0], args[1], args[2], args[3], padding, bias)
        ...
        # final layer
        lc = LocallyConnectedLayer1d(
            input_dim[j+1], output_dim[j+1], kernel_size[j+1], stride[j+1], padding, bias)

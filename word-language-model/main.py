...
if not args.test:
    ...
    model = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout,
                           args.tied, args.lcrnn, args.lc_kernel_size, args.lc_n_layers, args.lc_activation)
    if args.cuda:
        ...
        if args.guess:
            ...
            for g in range(args.guessn):
                ...
                if args.guessscores:
                    ...
                    outputguesses.append("{:.3f}".format(
                        float(guessscores[corpuspos][g])/float(guessscores[corpuspos][0])))
                    ...
                    outputguesses.append("{:.3f}".format(
                        math.exp(float(nn.functional.log_softmax(guessscores[corpuspos], dim=0)[g]))))
            outputguesses = ' '.join(outputguesses)
            print(str(word)+' '+str(sentid)+' '+str(corpuspos)+' '+str(len(word)) +
                  ' '+str(float(surp))+' '+str(float(Hs[corpuspos]))+' '+str(outputguesses))
        else:
            print(str(word)+' '+str(sentid)+' '+str(corpuspos)+' ' +
                  str(len(word))+' '+str(float(surp))+' '+str(float(Hs[corpuspos])))
        ...
        for i in range(0, lm_data_source.size(0) + ccg_data_source.size(0) - 1, args.bptt):
            ...
            if i > lm_data_source.size(0):
                data, targets = get_batch(
                    ccg_data_source, i - lm_data_source.size(0))
            # LM
            else:
...
def train():
    ...
    # random.seed(10)
    order = list(range(0, train_lm_data.size(0) +
                       train_ccg_data.size(0) - 1, args.bptt))
    random.shuffle(order)
    # enumerate(range(0, train_lm_data.size(0) + train_ccg_data.size(0) - 1, args.bptt)):
    for batch, i in tqdm(enumerate(order)):
        # TAG
        if i > train_lm_data.size(0):
            data, targets = get_batch(
                train_ccg_data, i - train_lm_data.size(0))
        # LM
        else:
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            logging.info('| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.2f} | ms/batch {:5.2f} | '
                         'loss {:5.2f} | ppl {:8.2f}'.format(
                             epoch, batch, (len(train_lm_data) +
                                            len(train_ccg_data)) // args.bptt, lr,
